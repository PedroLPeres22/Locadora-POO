...
public class CrudCliente {
    ...
    public static Cliente encontrarCliente(String telefone) {
        try {
            ...
            if (doc.exists()) {
                return doc.toObject(Cliente.class); // Converte o documento para Cliente e retorna
            } else {
                // Se o documento não existir, informa que o cliente não foi encontrado e
                // retorna null
    ...
    // Edita informações de um cliente já existente utilizando seu telefone como
    // referência
    ...
    public static void atualizarLocados(Cliente cliente) {
        try {
            db.collection("clientes")
                    .document(cliente.getTelefone())
                    .update("locados", cliente.getLocados())
                    .get();
            JOptionPane.showMessageDialog(null, "Locados atualizados com sucesso", "Atualização",
                    JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Erro ao atualizar locados: " + e.getLocalizedMessage(),
                    "ERRO!", JOptionPane.WARNING_MESSAGE);
        }
    }
